...
int main(int argc, char* argv[])
{
	...
	if (rank != 0) {
		...
		if (choise == 1) {
			...
			fscanf(inputPtr, "%d %d", &r1, &c1);
			mat1 = (int**)malloc(r1 * sizeof(int*));
			for (int i = 0; i < r1; i++)
				mat1[i] = (int*)malloc(c1 * sizeof(int));
			for (int i = 0; i < r1; i++) {
				for (int j = 0; j < c1; j++) {
					fscanf(inputPtr, "%d", &mat1[i][j]);
				}
			}
			// second matrix : read dimension , create matrix , read values
			fscanf(inputPtr, "%d %d", &r2, &c2);
			mat2 = (int**)malloc(r2 * sizeof(int*));
			for (int i = 0; i < r2; i++)
				mat2[i] = (int*)malloc(c2 * sizeof(int));
			for (int i = 0; i < r2; i++) {
				for (int j = 0; j < c2; j++) {
					fscanf(inputPtr, "%d", &mat2[i][j]);
				}
			}
			// close file
			fclose(inputPtr);
		...
		if (choise != 1) {
			...
			fprintf(outputPtr, "Result Matrix is (%d*%d):\n", r1, c2);
			for (int i = 0; i < r1; i++) {
				for (int j = 0; j < c2; j++)
					fprintf(outputPtr, "%d ", finalMat[i][j]);
				fprintf(outputPtr, "\n");
			}
			// close the file
			fclose(outputPtr);
